...
?> **Update**  
Since establishing my baseline, [Mint has migrated to using the HWE Kernel by default](https://blog.linuxmint.com/?p=4860)! This really does resolve my biggest problem with Mint and has me excited for the future of the project as they further modernize with Wayland support.
...
The only thing I really didn't like that I needed to deal with in setup was enabling unverified Flatpaks. I don't think Mint is necessarily wrong for wanting to disable them, but I do disagree with how strongly they advise against it and, more importantly, I believe it's something that the user should be prompted with up front. Similar to how Fedora handles third-party repositories, I think Mint should be prompting the user during installation or as a system report (similar to how they prompt users to set up system backups or the NVIDIA proprietary drivers). As is, the current solution could cause confusion from users who have been told "yes, this app is available and should just be in your software manager" and then they don't find it because of a setting no one told them about. I don't think this is by any means a deal breaker for my recommendations, but I do think having a prompt like that and softening the language a little bit (it's a reasonable concern, but most unverified Flatpaks are safe) would go a long way in helping users understand what will and won't show up in the Software Manager.
...
The last issue I encountered wasn't so much an error as much as functionality Mint just doesn't support. I decided to try connecting my Meta Quest 3 to Steam VR, both through Steam Link VR and a wired USB-C connection. Not much to say other than that it didn't really work. Hard to blame this on Mint though, since I had the same problem on other distros. Mint may still have a part in it from what I've read about it not supporting DRM leasing, but I do believe that the Meta Quest 3 may have more to do with it. I did also look into ALVR, but didn't want to try going through all that setup when SteamVR *might* have worked on other distros (spoiler alert: it didn't).