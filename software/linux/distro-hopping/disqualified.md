...
I'm not a big fan of Canonical packaging many Snaps independent of developer builds, as separating packaging from the application development can lead to issues being misdirected and cause general confusion around support (ex. [Steam Snap](https://www.omgubuntu.co.uk/2024/01/valve-dont-recommend-ubuntu-steam-snap) and [OBS on Fedora Flatpak](https://gitlab.com/fedora/sigs/flatpak/fedora-flatpaks/-/issues/39)). This is especially bad given Canonical marks anything they build with a "verified account" badge that makes it look like it is more official (ex. as of writing, [the Steam Snap](https://snapcraft.io/steam) still has that badge, despite Valve telling people not to use it).  
...
* Focus on the use case that Snaps are better suited to fill (lower level components) and provide a better user experience in their distros by using Flatpaks for use cases they are better suited to (GUI applications).
  * Honestly, I would really like to see this option, as Flatpaks for GUI apps and Snaps (with a rework to the repository model) for CLI tools could be a great way to solve some of my problems with [Immutable distros](#immutable-distros-and-elementary-os).
...
?> **Note:** I did also briefly look at [Kubuntu](https://kubuntu.org/) in a VM as well to see if the Snap issue persisted for [the official flavors](https://ubuntu.com/desktop/flavors), and found that, while it is extremely easy to re-enable Flatpaks and Flathub (literally just two buttons in the Discover settings), you still need to know to do that without anything prompting you to do so. If there weren't alternatives (like Tuxedo OS being based on Kubuntu) that solve this problem, I might have considered that good enough. However those alternatives do exist, so I'm also going to skip trying the Ubuntu flavors as well.
