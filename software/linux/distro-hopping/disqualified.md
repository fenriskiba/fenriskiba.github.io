...
## Ubuntu
Let's start with the obvious omission from my list: [Ubuntu](https://ubuntu.com/). I did look at Ubuntu in a VM and I do think it has a lot of advantages, but I think there are enough issues that I'm not interested in dealing with it and I don't feel comfortable recommending it.
...
In addition, Canonical's stance is contributing to a commonly referenced issue with the Linux desktop: "the fractured experience". In general, I fully believe that the issue of fracturing is often overblown, since most instances of "fracturing" are really "supporting a different use case/type of user". However, there is a balance that needs to be struck in the desktop space to avoid confusion and improve support and I do believe that Snaps negatively influence that balance.  
Ubuntu is the only major distro not to have adopted Flatpak, and it's clearly because they have a vested interest and don't want to accept that they've lost. If the problem were really about any technical issues, then they would either:
* Focus on the use case that Snaps are better suited to fill (lower level components) and provide a better user experience by using Flatpaks for use cases they are better suited to (Gui applications).
